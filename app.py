...
app = FastAPI(title="Primitive Recommender")

users: Dict[str, dict] = {}
items: Dict[str, Item] = {}
ratings: Dict[str, List[RatingOut]] = {}
...
def _seed_items():
    ...
    for title, kind, genre, author in sample_items:
        iid = str(uuid4())
        items[iid] = Item(id=iid, title=title, kind=kind, genre=genre, author=author)
...
def register(user_in: UserIn):
    ...
    user_id = str(uuid4())
    user = user_in.model_dump()  # pydantic v2 compliant
...
def recommend(user=Depends(get_current_user)):
    ...
    if user_ratings:
        ...
        candidate_items = [i for i in items.values() if i.genre == top_genre and i.id not in rated_ids]
    else:
        candidate_items = []

    if not candidate_items:
        candidate_items = [i for i in items.values() if i.id not in rated_ids]
